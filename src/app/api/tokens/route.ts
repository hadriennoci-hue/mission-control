...
interface OpenClawRollup {
  per_agent_model?: RollupEntry[]
  per_agent?: RollupEntry[]
}
...
async function loadTokenData(): Promise<TokenUsageRecord[]> {
  ...
  // Then try loading latest OpenClaw rollup generated by scripts/generate-openclaw-token-rollup.sh
  try {
    const rollupRecords = await loadTokenDataFromRollup()
    if (rollupRecords.length > 0) return rollupRecords
  } catch {
    // Ignore rollup parse issues and continue fallback
  }
...
function deriveRepoRootFromOpenClawConfigPath(): string | null {
...
async function findLatestRollupPath(): Promise<string | null> {
  const explicit = process.env.OPENCLAW_TOKEN_ROLLUP_PATH?.trim()
  if (explicit) return explicit

  const repoRoot = deriveRepoRootFromOpenClawConfigPath()
  if (!repoRoot) return null

  const costsDir = join(repoRoot, 'analytics', 'costs')
  const entries = await readdir(costsDir)
  const candidates = entries.filter((n) => n.startsWith('openclaw-token-rollup-') && n.endsWith('.json'))
  if (candidates.length === 0) return null

  let latestPath: string | null = null
  let latestMtime = -1
  for (const name of candidates) {
    const filePath = join(costsDir, name)
...
async function loadTokenDataFromRollup(): Promise<TokenUsageRecord[]> {
  const rollupPath = await findLatestRollupPath()
  if (!rollupPath) return []

  const raw = await readFile(rollupPath, 'utf-8')
  const parsed = JSON.parse(raw) as OpenClawRollup
  const mtime = (await stat(rollupPath)).mtimeMs || Date.now()

  const records: TokenUsageRecord[] = []

  // Preferred: per_agent_model gives accurate split by agent and model.
  if (Array.isArray(parsed.per_agent_model) && parsed.per_agent_model.length > 0) {
    for (const entry of parsed.per_agent_model) {
      ...
      records.push({
        id: `rollup-${agent}-${model}-${mtime}`,
        model,
        sessionId: `${agent}:rollup`,
        timestamp: mtime,
        inputTokens,
        outputTokens,
        totalTokens,
  ...
  // Backward-compatible fallback: per_agent only (model not available).
  if (Array.isArray(parsed.per_agent) && parsed.per_agent.length > 0) {
    for (const entry of parsed.per_agent) {
      const agent = entry.key || entry.agent || 'unknown'
      const inputTokens = Number(entry.input_tokens || 0)
      const outputTokens = Number(entry.output_tokens || 0)
      const totalTokens = Number(entry.total_tokens || (inputTokens + outputTokens))
      const model = 'rollup/unknown-model'
      const costPer1k = getModelCost(model)
      const cost = (totalTokens / 1000) * costPer1k

      records.push({
        id: `rollup-${agent}-${mtime}`,
        model,
        sessionId: `${agent}:rollup`,
        timestamp: mtime,
        inputTokens,
